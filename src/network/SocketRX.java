...
public class SocketRX {
    ...
    public void setSocketRX(HttpServletRequest request){
        ...
        System.out.println("number is: " +(int)request.getSession().getAttribute(AttributeNames.UAV_HOST_PORT_NUMBER) );
        try {
            socket = new Socket((String)request.getSession().getAttribute(AttributeNames.UAV_HOST_ADDRESS),
                    (int)request.getSession().getAttribute(AttributeNames.UAV_HOST_PORT_NUMBER));
